...
export const SkiLengthForm: React.FC<SkiLengthProps> = ({ className, onFormSubmit }) => {
  ...
  return (
    <form className={`${className || ''} w-[800px]`} onSubmit={handleSubmit(onSubmit)}>
			...
			<Button className="border-2 border-green-400 rounded-md py-2 px-4 hover:bg-green-400 cursor-pointer" disabled={isDisabled}>Get recommended ski length</Button>
		</form>
  )
}